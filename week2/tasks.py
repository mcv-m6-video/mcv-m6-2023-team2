...
def task1(args):
    ...
    if args.channels:
        args_t1['channels'] = args.channels
...
def task4(args):
    def objective(trial):
        alpha = trial.suggest_float('alpha', 0, 10, step=0.5)
        rho = trial.suggest_float('rho', 0, 1)
        color_space = trial.suggest_categorical('color_space', ['RGB', 'LAB', 'HSV', 'YCrCb'])
        channels = trial.suggest_categorical('channels', ['all', '0', '1', '2'])
        voting = trial.suggest_categorical('voting', ['unanimous', 'simple'])
        args.alpha = alpha
        args.rho = rho
